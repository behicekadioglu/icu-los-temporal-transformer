import torch, os, json, numpy as np, pandas as pd, xgboost as xgb, time
...
def run_ablation(mode='top_k'):
    ...
    X_t, X_s, y, f_names = np.load('data/processed/X_temporal_raw.npy'), np.load('data/processed/X_static_raw.npy'), np.load('data/processed/y.npy'), np.load('data/processed/feature_names.npy')
    ...
    for fold, (tr_idx, val_idx) in enumerate(kf.split(X_t)):
        ...
        if m['MAE'] < best_mae:
            ...
            with open(f'results/models/config_{mode}.json', 'w') as f: json.dump({'params': m_params, 'input_dim': xt_tr.shape[2], 'static_dim': xs_tr.shape[1]}, f)
