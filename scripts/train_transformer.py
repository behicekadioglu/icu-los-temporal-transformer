...
class MIMICDataset(Dataset):
    ...
    def __getitem__(self, idx): return self.X_t[idx], self.X_s[idx], self.y[idx]
...
class TemporalTransformer(nn.Module):
    def __init__(self, input_dim, static_dim, d_model, nhead, num_layers, dropout=0.1):
        ...
        self.regression_head = nn.Sequential(nn.Linear(d_model + static_dim, d_model // 2), nn.ReLU(), nn.Dropout(dropout), nn.Linear(d_model // 2, 1))
...
def evaluate_metrics(model, loader, device):
    ...
    return {'MAE': mean_absolute_error(all_y, all_p), 'RMSE': np.sqrt(mean_squared_error(all_y, all_p)), 
            'R2': r2_score(all_y, all_p), 'MedAE': median_absolute_error(all_y, all_p), 'Inf_Time': inf_time}
    ...
    for tr_idx, val_idx in kf.split(X_t):
        ...
        for e in range(10):
            ...
            for xt, xs, target in train_l:
                optimizer.zero_grad(); nn.HuberLoss()(model(xt.to(device), xs.to(device)), target.to(device)).backward(); optimizer.step()
        maes.append(evaluate_metrics(model, val_l, device)['MAE'])
    return np.mean(maes)

if __name__ == "__main__":
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    X_t, X_s, y = np.load('data/processed/X_temporal_raw.npy'), np.load('data/processed/X_static_raw.npy'), np.load('data/processed/y.npy')
    ...
    for fold, (tr_idx, val_idx) in enumerate(kf.split(X_t)):
        ...
        if m['MAE'] < best_mae:
            ...
            with open('results/models/model_config.json', 'w') as f: json.dump({'params': best_p, 'input_dim': F, 'static_dim': X_s.shape[1]}, f)
